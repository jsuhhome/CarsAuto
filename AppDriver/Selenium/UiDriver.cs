...
namespace AppDriver.Selenium
{
    public class UiDriver
    {
        ...
        private TimeSpan timeout = new TimeSpan(0, 3, 00);
        ...
        public Dictionary<string, IWebElement> GetDicOfElementsBySubElementText(By subElementInElement,
            By elementsForDic)
        {
            By RootElement = By.CssSelector(@"body");
            return GetDicOfElementsBySubElementText(subElementInElement, elementsForDic, this.FindElement(RootElement));
        }
        ...
        public IWebElement FindElement(By by, By byChild)
        {
            try
            {
                IWebElement parent = driver.FindElement(by);
        ...
        protected void WaitTextOnAttribute(By by, string attribute, string text)
        {
            ...
            wait.Until<bool>(
                d =>
                {
                    try
                    {
                        string attribText = GetAttribute(by, attribute);
        ...
        {
            By modal = By.CssSelector("iframe.cboxIframe[src*='" + ModalName + "']");
        ...
        {
            bool jQueryDefined = (bool)javascript.ExecuteScript("return typeof jQuery != 'undefined'");
            if (jQueryDefined)
            {
                WaitForJQueryLoad(wait, javascript);
            }
        }
        ...
        private void WaitForJQueryLoad(WebDriverWait wait, IJavaScriptExecutor javascript)
        {
            try
            {
                bool jqueryReady = (bool)javascript.ExecuteScript("return jQuery.active==0");
